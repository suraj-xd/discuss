    ...
    <div className=" card hover:bg-gray-100 text-xs sm:text-base rounded-r-xl border-blue-200 hover:border-l-4 shadow-md shadow-gray-300 cursor-pointer">
      ...
      <Link href={`/${post.username}/${post.slug}`}>
        <span >
          ...
          <p style={{ "width": "60%" }} className='p-2 flex-col whitespace-wrap text-gray-600 '>{post.description}
          </p>
          <ul className='decoration-none text-sm flex flex-wrap overflow-hidden mr-2 w-[80%]  '>
          {post?.tag?.map((hash) => {
            return (
              <>
            ...
            <span className=" flex flex-row mt-1"> {post.heartCount || 0} <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="w-6 h-6">
