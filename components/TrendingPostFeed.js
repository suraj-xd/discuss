...
function PostItem({ post, admin = false }) {
  ...
  return (
    <div style={{ "margin": "5px", "backgroundColor": "#002D62" }} className="card  text-xs  mr-2 bg-[blue] sm:text-base border-blue-200 hover:border-l-4 shadow-md shadow-gray-300 cursor-pointer">
      ...
      <Link href={`/${post.username}/${post.slug}`}>
        <span >
          <h1 className='border-l-2 max-h-20 text-base sm:text-xl text-white   border-green-400 pl-2 whitespace-nowrap mt-2 mainfont'>
            <a className='' >{post.title}</a>
          </h1 >
          {/* <p style={{ "width": "60%" }} className='p-2 overflow-hidden whitespace-nowrap text-gray-300 '>{post?.description}
          </p> */}
          <ul className='decoration-none mr-2 overflow-hidden '>
            
            <span className="flex  text-white mt-2"> {post.heartCount || 0} <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="w-6 h-6">
